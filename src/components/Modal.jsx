...
const handleCurrency = async (event) => {
  ...
  setBuyFlight({
    ...
    finalPrice: Math.round(event.target.value === 'COP' ? COPrate * detail.price : detail.price),
    indatumMiles: gainedMiles})
}

const handleSaveData = () => {
  localStorage.setItem("indatum-flights-reservation", JSON.stringify(buyFlight));
};
  ...
  return (
    <div>
      ...
      <h4>Final price: {buyFlight.finalPrice !== '' ? <p>{buyFlight.currency}: {buyFlight.finalPrice}</p>: <p>{buyFlight.currency}: {detail.price}</p>}</h4>
      <p>You have gained {buyFlight.indatumMiles} miles</p>
      <p>Antes de comprar selecciona el tipo de moneda</p>
        <button disabled={buttonBuy} onClick={handleSaveData}>Comprar</button>
